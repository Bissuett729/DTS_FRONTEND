...
export class DepartmentRequestService {
  ...
  getDepartments(loading = true) {
    ...
    this.http.get(`${this.dtsURL}/v1/department`)
      .pipe(finalize(() => { if (loading) this.departmentState.loadingDepartments.set(false); }))
      .subscribe({
        next: (res: any) => {
  ...
  createDepartment(payload: { department: string; description?: string }) {
    ...
    this.http.post(`${this.dtsURL}/v1/department`, payload)
      .pipe(finalize(() => this.departmentState.loadingCreateDepartment.set(false)))
      .subscribe({
        next: () => { this.alert.success('Departamento creado exitosamente'); this.getDepartments(false); },
        error: () => this.alert.error('Error al crear el departamento'),
      });
  }

  addReason(deptId: string, reason: string) {
    this.http.post(`${this.dtsURL}/v1/department/${deptId}/reasons`, { reason })
      .subscribe({
        next: () => { this.alert.success('Razón agregada'); this.getDepartments(false); },
        error: () => this.alert.error('Error al agregar la razón'),
      });
  }

  updateReason(deptId: string, oldReason: string, newReason: string) {
    this.http.delete(`${this.dtsURL}/v1/department/${deptId}/reasons/${encodeURIComponent(oldReason)}`)
      .subscribe({
        next: () => {
          this.http.post(`${this.dtsURL}/v1/department/${deptId}/reasons`, { reason: newReason })
            .subscribe({
              next: () => { this.alert.success('Razón actualizada'); this.getDepartments(false); },
  ...
  removeReason(deptId: string, reason: string) {
    this.http.delete(`${this.dtsURL}/v1/department/${deptId}/reasons/${encodeURIComponent(reason)}`)
      .subscribe({
        next: () => { this.alert.success('Razón eliminada'); this.getDepartments(false); },
  ...
  updateDepartment(deptId: string, payload: { department: string; description?: string }) {
    ...
    this.http.patch(`${this.dtsURL}/v1/department/${deptId}`, payload)
      .pipe(finalize(() => this.departmentState.loadingUpdateDepartment.set(false)))
      .subscribe({
        next: () => { this.alert.success('Departamento actualizado'); this.getDepartments(false); },
  ...
  deleteDepartment(deptId: string) {
    ...
    this.http.delete(`${this.dtsURL}/v1/department/${deptId}`)
      .pipe(finalize(() => this.departmentState.loadingDeleteDepartment.set(false)))
      .subscribe({
        next: () => { this.alert.success('Departamento eliminado'); this.getDepartments(false); },

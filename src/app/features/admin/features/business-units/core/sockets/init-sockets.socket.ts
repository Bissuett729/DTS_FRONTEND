...
import { BusinessUnitsSocketManager } from '../../../../../../shared/services/business-units-socket-manager.service';

@Injectable({ providedIn: 'root' })
export class InitBusinessUnitsSockets {
  private readonly businessUnitsStateService = inject(BusinessUnitStateService);
  private readonly businessUnitsSocketManager = inject(BusinessUnitsSocketManager);
  ...
  InitSockets() {
    ...
    this.businessUnitsSocketManager.onBusinessUnitCreated((response) => {
    ...
    this.businessUnitsSocketManager.onBusinessUnitUpdated((response) => {
      const payload = this.extractPayload<Partial<IBusinessUnit> & { businessUnitId?: string }>(
        response,
      );
      ...
      if (payload?._id || payload?.businessUnitId) {
        ...
        this.businessUnitsStateService.updateBusinessUnit({
          _id: payload.businessUnitId,
          ...payload,
        });
      }
    });

    this.businessUnitsSocketManager.onBusinessUnitDeleted((response) => {
